                    ...
                    <tr key={j._id}>
                      ...
                      <td className="px-4 py-4 text-sm whitespace-nowrap">
                        <div className="flex items-center gap-x-2">
                          <p
                            className={`px-3 py-1  ${
                              j.category === 'Web Development' &&
                              'text-blue-500 bg-blue-100/60'
                            } ${
                              j.category === 'Graphics Design' &&
                              'text-emerald-500 bg-emerald-100/60'
                            } ${
                              j.category === 'Digital Marketing' &&
                        ...
                        >
                          <span
                            className={`h-1.5 w-1.5 rounded-full ${
                              j.status === 'Pending' && 'bg-yellow-500'
                            } ${
                              j.status === 'In Progress' && 'bg-blue-500'
                            } ${j.status === 'Complete' && 'bg-green-500'} ${
                              j.status === 'Complete' && 'bg-green-500'
                            } ${j.status === 'Rejected' && 'bg-red-500'} `}
                          ></span>
                          <h2 className='text-sm font-normal '>{j.status}</h2>
                        </div>
                      </td>
                      <td className="px-4 py-4 text-sm whitespace-nowrap">
                        <button
                          disabled={j.status !== "In Progress"}
                          onClick={() => handleStatus(j._id, "Complete")}
                          title="Mark Complete"
